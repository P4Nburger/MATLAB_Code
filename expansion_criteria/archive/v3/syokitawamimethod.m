% =========================================================================
% 2自由度 拡張シミュレーション (v2改: 初期たわみ算出メソッド変更版)
% =========================================================================
clear; clc; close all;
...
% 1. パラメータ設定
% -------------------------------------------------------------------------
tspan = [0 1.0];    % [s] 時間軸は0～1.0秒
t_sound_on = 0.2;   % [s] 音波開始

% --- ★ 初期たわみメソッドの変更 ★ -----------------------------------
% 以前: z_target_initial を直接指定していましたが、
% 今回: 「フレーム押し込み量」から「有効形状定数」を使って算出します。

x_push = 0.07e-3;    % [m] フレームの押し込み量 (実験パラメータ)
C_eff = 0.2794;     % 有効形状定数 (ひし形モデル, kotei_LikoruS_danmen.mより)

% たわみ量の計算 (単位をmmに合わせて計算してからmに戻す)
% 公式: delta = sqrt(2 * x / C)
...
% --- 平衡点の計算 ---
% ここは v2 のロジックを維持 (算出された z_target_initial を使用)
z = z_target_initial;
...
hold off;
ylabel('Z 変位 [mm]'); 
title(sprintf('膜のたわみ (初期: %.3f mm)', z_target_initial*1000)); 
grid on;
...
title('振動の激しさと剛性の変化'); 
grid on; xlabel('時間 [s]');
legend('振動の激しさ A', 'しきい値', '剛性 k1', 'Location', 'best');
